...
4. **豆包 Seedance 视频生成测试（Doubao Video）**：
   ...
   - **接口路径与路由兼容**：提供快速预设路径切换（自动探测、官方标准 `/api/v3/contents/generations/tasks`、无 `/api/v3`、网关 `/v1` 等）与自定义路径覆盖输入，服务端内置 URL 智能前缀规范化与去重（避免 `/api` Base URL 导致 `/api/api/v3` 404）；
   - **右侧实时报文回显与单跑管道**：提供「请求 JSON / 轮询响应 / 提交响应」多维实时回显与一键复制；支持独立「单跑提交」、「单跑轮询」、独立「⚡ 立即查询状态」及完整测试管道；
   - **独立视频测试报告**：视频模块生成与导出的 PDF / HTML 报告与文本模型报告完全解耦独立，专属渲染视频执行流水线与参数详情；执行完毕后内嵌 HTML5 播放器直接播放。
5. **动态基准打分与报告导出**：
...
- [x] **上游同步:** 已在公司最新主干 `d5eb67d32` 之上完成基线同步，无任何遗漏或冲突。
- [x] **本地验证:** 已在本地运行并通过测试与校验：
  - `go test -v ./service/suppliertest/... ./controller ./model -count=1` 100% 通过
  - `bunx oxlint -c .oxlintrc.json src/features/supplier-test` 0 warnings, 0 errors
  - `bun run typecheck` (`tsgo -b`) 编译通过
  - `bun run test -- --pool=threads src/features/supplier-test/` 26/26 测试通过
- [x] **安全合规:** 代码中无敏感凭据，且符合项目代码规范。

## 📸 运行证明 / Proof of Work

- `go test -v ./service/suppliertest/... ./controller ./model -count=1` 全通过
